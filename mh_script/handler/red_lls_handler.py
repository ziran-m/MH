    ...
    def do(self, region: ScreenRegion):
        ...
        for _ in range(5):
            ...
            if pos is not None:
                self.ocrPlayer.touch(pos, True, None)
                self.delay()
            pos = self.ocrPlayer.wait_find_by_pic_first(region, "lls.push")
            if pos is not None:
                self.ocrPlayer.touch(pos, True, None)
                self.delay()

            pos_list = self.ocrPlayer.find_by_pic(None, "lls.accept")
            for pos in pos_list:
                self.ocrPlayer.touch(pos, True, None)
                self.delay()
            while True:
                global_log.info("[玲珑石] 流程中")
                pos_list = self.ocrPlayer.find_by_pic(None, "lls.end")
                ...
                time = self.ocrPlayer.wait_find_by_pic_first(region, "common.time")
                if time is not None:
                    self.ocrPlayer.touch(time, True, None)
                    self.delay()
                    self.ocrPlayer.touch(time, True, None)
                    self.delay()
                    pos = self.ocrPlayer.wait_find_by_pic_first(region, "lls.accept")
                    if pos is not None:
                        self.ocrPlayer.touch(pos, True, None)
                        self.delay()
